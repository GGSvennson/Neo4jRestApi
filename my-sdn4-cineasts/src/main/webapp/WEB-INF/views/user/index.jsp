...
<%--@elvariable id="recommendations" type="java.util.List<org.neo4j.cineasts.domain.Movie>"--%>
<%--@elvariable id="recommendation" type="org.neo4j.cineasts.domain.Movie"--%>
<html>
  <head>
    <title>Profile</title>
  </head>
  <body>
    ...
    <div class="span-5">
      <div class="profile-header">
        <div class="profile-image"><img src="<c:url value="/images/users/${user.login}.jpg" />" /></div>
    ...
    <c:choose>
        <c:when test="${not empty user.friends}">
            ...
            <c:forEach items="${user.friends}" var="friend">
                <li>
                    <a class="friend-image" href="<c:url value="/user/${friend.login}" />"><img src="<c:url value="/images/users/${friend.login}.jpg" />" /></a>
        ...
        <c:otherwise>
	        <form action="<c:url value="/user/list" />" method="get">
	            <input type="submit" value="Look for friends"/>
	        </form>
            <c:choose>
                              			...
                              			<c:forEach items="${recommendations}" var="recommendation">
		                      				<c:choose>
		                      					<c:when test="${not empty recommendation}">
		                      						<c:set var="tagline" value="${recommendation.tagline}"/>
		                      					</c:when>
		                      					<c:otherwise>
		                      						<c:set var="tagline" value="${movie.tagline}"/>
		                      					</c:otherwise>
		                      				</c:choose>
		                      				<form method="get" action="<c:url value="/movies/${movie.id}/recommendations" />">
								            	<h4>Recommend &quot;${movie.title}&quot; to your friends saying:
								                <input type="hidden" value="${stars}" name="rated" id="rated"/>
								                <input type="text" size="100" name="tagline" value="${tagline}"/>

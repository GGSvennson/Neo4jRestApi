...
import org.neo4j.cineasts.domain.Recommendation;
...
public class MovieController {
    ...
    @RequestMapping(value = "/movies/{movieId}/recommendations", method = RequestMethod.GET, headers = "Accept=text/html")
    public String doRecommendation(Model model, @PathVariable String movieId,
    		@RequestParam(value = "rated", required = true) Integer stars,
    		@RequestParam(value = "tagline", required = false) String tagline) {
        Movie movie = IteratorUtil.firstOrNull(findMovieByProperty("id", movieId));
        User user = userRepository.getUserFromSession();
        if (user != null && movie != null && stars > 2) {
        	Recommendation userRecommendation = new Recommendation();
        	userRecommendation.setUser(user);
        	userRecommendation.setMovie(movie);
        	if(tagline != null)
        		userRecommendation.setTagline(tagline);
        	user.recommend(movie, tagline);
			userRepository.save(user);
        }
        if (user != null) {
    ...
    @RequestMapping(value = "/movies/all", method = RequestMethod.GET, headers = "Accept=text/html")
    public String findAll(Model model, @RequestParam("p") String page, @RequestParam("q") String flag) {
    	int k = Integer.valueOf(page).intValue();
    	if(flag.equals("0"))
    		k++;
    	...
    	Page<Movie> movies = movieRepository.findAll(new PageRequest(k, 5));
    	model.addAttribute("movies", IteratorUtil.asCollection(movies));
    	model.addAttribute("page", String.valueOf(k));
        addUser(model);
        return "/movies/all";
    }

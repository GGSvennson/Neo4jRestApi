...
public interface MovieRepository extends GraphRepository<Movie> {
    ...
    */
    @Query("MATCH (user:User)-[rec:RECOMMENDED]->(movie:Movie), " +
    		"(user)-[rat:RATED]->(movie) " +
    		"WHERE user.login = {0} " +
    		"AND rat.stars > 2 " +
    		"WITH movie, rec.tagline AS tagline, toInt(round(avg(rat.stars))) AS rating, count(*) AS cnt " +
    		"ORDER BY rating DESC, cnt DESC " +
    		"RETURN movie.id as movieId, movie.title AS title, tagline AS tagline, rating AS rating LIMIT 10")
